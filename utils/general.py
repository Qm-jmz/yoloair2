...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ECIoU=False, FocalLoss_= 'none', eps=1e-7):
    ...
    iou = inter / union
    if CIoU or DIoU or GIoU or EIoU or SIoU or ECIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1mg
            ...
            with torch.no_grad():
                alpha = v / (v - iou + (1 + eps))
            if FocalLoss_ == 'ECIoU':
                # print(' use focal-ECiou 🚀')
                return iou - (rho2 / c2 + w_dis/cw2+h_dis/ch2 + v * alpha), (inter/(union + eps)) ** 0.5
