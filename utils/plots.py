...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(), save_dir=''):
    ...
    ax = ax.ravel()
    s = ['Box', 'Objectness', 'Classification', 'Precision', 'Recall', 'F1',
        'val Box', 'val Objectness', 'val Classification', 'mAP@0.5', 'mAP@0.5:0.95', 'mAP@0.75']
    if bucket:
    ...
    for fi, f in enumerate(files):
        try:
            results = np.loadtxt(f, usecols=[2, 3, 4, 8, 9, 13, 14, 15, 16, 10, 11, 12], ndmin=2).T
            n = results.shape[1]  # number of rows
            x = range(start, min(stop, n) if stop else n)
            for i in range(12):
                y = results[i, x]
